...
<fieldset class="layui-elem-field">
	...
	<div class="layui-field-box">
			<div class="layui-form-item">
			 <div class="layui-col-md3">{{ $order->labels["order_no"] }}:{{ $order->order_no }}</div>
			 <div class="layui-col-md3">{{ $items->labels["title"] }}:{{ $items->title }}</div>
			 <div class="layui-col-md3">{{ $order->labels["wh_items_quantity"] }}:{{ $order->wh_items_quantity }}</div>
			  <div class="layui-col-md3">{{ $order->labels["out_quantity"] }}:{{ $order->out_quantity }}</div>
			</div>
	</div>
</fieldset>
<fieldset class="layui-elem-field">
...
<div class="my-btn-box">
	...
	<span class="fl">
		<a class="layui-btn layui-btn-primary  layui-btn-sm" href="{{ url('admin/orderOut/index') }}"><i class="fa fa-mail-reply"></i> 返回</a>
...
<script type="text/html" id="setIn">
 [{{'{'.'{'.'d.wh_items_num'.'}'.'}'}}] &nbsp;&nbsp;<span class="layui-badge">点击出库</span>
</script>
@endsection
	...
	table.on("tool(dataTable)", function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		...
		if(layEvent === "setIn"){
			layer.open({
				  ...
				  content: ['{{ url("admin/orderOut/setOutUnit") }}?orderId={{$orderId}}&unitId='+data.id, 'no']
				});
