...
<div class="my-btn-box">
	<span class="fl">
		...
		<a class="layui-btn btn-add btn-default layui-btn-sm" id="btn-add" href="{{ URL('admin/wapMenu/create') }}">添加</a>
...
// ,{field: "pid", title: "{{ $model->labels["pid"] }}",  sort: true}
,{field: "menu_name", title: "{{ $model->labels["menu_name"] }}",  sort: true}
// ,{field: "menu_type", title: "{{ $model->labels["menu_type"] }}",  sort: true}
// ,{field: "menu_site", title: "{{ $model->labels["menu_site"] }}",  sort: true}
// ,{field: "menu_path", title: "{{ $model->labels["menu_path"] }}",  sort: true}
// ,{field: "target", title: "{{ $model->labels["target"] }}",  sort: true}
// ,{field: "remarks", title: "{{ $model->labels["remarks"] }}",  sort: true}
,{field: "menu_status_text", title: "{{ $model->labels["menu_status"] }}",  sort: true}
,{field: "order_id", title: "{{ $model->labels["order_id"] }}",  sort: true}
// ,{field: "group", title: "{{ $model->labels["group"] }}",  sort: true}
	...
	$("#btn-delete-all").on("click", function () {
		...
		$.ajax({ 
			type: "GET",
			url: "{{URL("admin/wapMenu/ajaxDestroy")}}/"+ids, 
			dataType: "json",
			success: function(data){
				if(data.code=="000")
				{
