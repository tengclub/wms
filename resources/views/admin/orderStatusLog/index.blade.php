			...
			<div class="layui-form-item">
    			...
    			<div class="layui-inline">
					...
					<div class="layui-input-inline">
						<input type="text" id="order_id" name="order_id" value="{{$model->order_id}}"  autocomplete="off" class="layui-input">
					</div>
				</div>
...
<div class="my-btn-box">
	<span class="fl">
		...
		<a class="layui-btn btn-add btn-default layui-btn-sm" id="btn-add" href="{{ URL('admin/orderStatusLog/create') }}">添加</a>
...
,{field: "order_id", title: "{{ $model->labels["order_id"] }}",  sort: true}
,{field: "order_status_id", title: "{{ $model->labels["order_status_id"] }}",  sort: true}
,{field: "order_status_title", title: "{{ $model->labels["order_status_title"] }}",  sort: true}
,{field: "status_value", title: "{{ $model->labels["status_value"] }}",  sort: true}
,{field: "object_type", title: "{{ $model->labels["object_type"] }}",  sort: true}
...
,{field: "create_time", title: "{{ $model->labels["create_time"] }}",  sort: true}
	...
	table.on("tool(dataTable)", function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		...
		if(layEvent === "del"){
			layer.confirm("真的删除吗？", function(index){
				...
				$.ajax({ 
					type: "GET",
					url: "{{URL("admin/orderStatusLog/ajaxDestroy")}}/"+data.id, 
					dataType: "json",
			...
			layer.msg("编辑操作");
			window.location.href="{{ url("admin/orderStatusLog/edit/") }}/"+data.id;
		}
	});
	// 刷新
	...
	$("#btn-delete-all").on("click", function () {
		...
		$.ajax({ 
			type: "GET",
			url: "{{URL("admin/orderStatusLog/ajaxDestroy")}}/"+ids, 
			dataType: "json",
			success: function(data){
				if(data.code=="000")
				{
