...
<fieldset class="layui-elem-field">
	<legend>{{ $group->group_name }}组 - wap菜单</legend>
...
,{field: "menu_id_text", title: "{{ $model->labels["menu_id"] }}",  sort: true}
		]]
		,size: "sm" 
		, id: "id"
		, method: "get"
	...
	table.on("tool(dataTable)", function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		...
		if(layEvent === "del"){
			layer.confirm("真的删除吗？", function(index){
				...
				$.ajax({ 
					type: "GET",
					url: "{{URL("admin/sysGroupWapMenu/ajaxDestroy")}}/"+data.id, 
					dataType: "json",
			...
			layer.msg("编辑操作");
			window.location.href="{{ url("admin/sysGroupWapMenu/edit/") }}/"+data.id;
		}
	});
	// 刷新
	...
	$("#btn-add").on("click", function () {
		...
		layer.open({
		  ...
		  content: ['{{ url("admin/wapMenu/group",['gid'=>$group->id]) }}', 'no']
		});
	});
	// 批量删除
	$("#btn-delete-all").on("click", function () {
		...
		$.ajax({ 
			type: "GET",
			url: "{{URL("admin/sysGroupWapMenu/ajaxDestroy")}}/"+ids, 
			dataType: "json",
			success: function(data){
				if(data.code=="000")
				{
