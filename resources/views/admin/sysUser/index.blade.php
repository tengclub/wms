...
<div class="my-btn-box">
	<span class="fl">
		...
		<a class="layui-btn btn-add btn-default layui-btn-sm" id="btn-add" href="{{ url('admin/sysUser/create') }}">添加</a>
...
layui.use("table", function(){
	...
	var tableIns = table.render({ //其它参数在此省略
		...
		,url: "{{ url("admin/sysUser/ajaxData") }}?"+$("form").serialize()
		, cols: [[ 
			{field: "id", title: "ID", width: 80, sort: true ,checkbox: true, }
			, {field: "user", title: "{{ $model->labels['user'] }}", width: 120, sort: true}
			, {field: "level_text", title: "{{ $model->labels['level'] }}", width: 180}
			, {field: "user_status_text", title: "{{ $model->labels['user_status'] }}", width: 180}
	...
	table.on("tool(dataTable)", function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		...
		if(layEvent === "del"){
			layer.confirm("真的删除吗？", function(index){
				...
				$.ajax({ 
					type: "GET",
					url: "{{URL("admin/sysUser/ajaxDestroy")}}/"+data.id, 
					dataType: "json",
...
// 			layer.msg("编辑操作");
			window.location.href="{{ url("admin/sysUser/edit/") }}/"+data.id;
		}
	});
	// 刷新
	...
	$("#btn-delete-all").on("click", function () {
		...
		$.ajax({ 
			type: "GET",
			url: "{{URL("admin/sysUser/ajaxDestroy")}}/"+ids, 
			dataType: "json",
			success: function(data){
				if(data.code=="000")
				{
