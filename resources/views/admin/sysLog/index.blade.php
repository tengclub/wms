...
layui.use("table", function(){
	...
	var tableIns = table.render({ //其它参数在此省略
		...
		,url: "{{ url("admin/sysLog/ajaxData") }}?"+$("form").serialize()
		, cols: [[ 
			{field: "id", title: "ID", width: 80, sort: true ,checkbox: true, }
			, {field: "data_time", title: "{{ $model->labels['data_time'] }}", width: 180, sort: true}
			, {field: "user", title: "{{ $model->labels['user'] }}", width: 180}
			, {field: "log_type_text", title: "{{ $model->labels['log_type'] }}", width: 180}
			, {field: "content", title: "{{ $model->labels['content'] }}", width: 180}
	...
	table.on("tool(dataTable)", function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		...
		if(layEvent === "del"){
			layer.confirm("真的删除吗？", function(index){
				...
				$.ajax({ 
					type: "GET",
					url: "{{URL("admin/sysLog/ajaxDestroy")}}/"+data.id, 
					dataType: "json",
...
// 			layer.msg("编辑操作");
			window.location.href="{{ url("admin/sysLog/edit/") }}/"+data.id;
		}
	});
	// 刷新
	...
	$("#btn-delete-all").on("click", function () {
		...
		$.ajax({ 
			type: "GET",
			url: "{{URL("admin/sysLog/ajaxDestroy")}}/"+ids, 
			dataType: "json",
			success: function(data){
				if(data.code=="000")
				{
