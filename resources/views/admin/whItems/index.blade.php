...
<div class="my-btn-box">
	<span class="fl">
		...
		<a class="layui-btn btn-add btn-default layui-btn-sm" id="btn-add" href="{{ URL('admin/whItems/create') }}">添加</a>
...
<script type="text/html" id="barWhUnit">
	<a lay-event="whUnit" title="查看">&nbsp;<i class="layui-icon">&#xe62d;</i>&nbsp;&nbsp;</a>
</script>
@endsection
...
,{field: "out_code", title: "{{ $model->labels["out_code"] }}",  sort: true}
,{field: "num", title: "库存数量",  sort: true}
,{field: "weight", title: "{{ $model->labels["weight"] }}",  sort: true}
,{field: "status_text", title: "{{ $model->labels["status"] }}",  sort: true}
, {fixed: "right", title: "仓位", width: 150, align: "center", toolbar: "#barWhUnit"} //
	...
	table.on("tool(dataTable)", function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		...
		if(layEvent === "del"){
			layer.confirm("真的删除吗？", function(index){
				...
				$.ajax({ 
					type: "GET",
					url: "{{URL("admin/whItems/ajaxDestroy")}}/"+data.id, 
					dataType: "json",
			...
			layer.msg("编辑操作");
			window.location.href="{{ url("admin/whItems/edit/") }}/"+data.id;
		} else if(layEvent === "whUnit"){
			layer.msg("查看仓位");
			window.location.href="{{ url("admin/whItems/whUnit/") }}/"+data.id;
		}
	});
	// 刷新
	...
	$("#btn-delete-all").on("click", function () {
		...
		$.ajax({ 
			type: "GET",
			url: "{{URL("admin/whItems/ajaxDestroy")}}/"+ids, 
			dataType: "json",
			success: function(data){
				if(data.code=="000")
				{
