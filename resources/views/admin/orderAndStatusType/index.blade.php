			...
			<div class="layui-form-item">
    			...
    			<div class="layui-inline">
					...
					<div class="layui-input-inline">
						<input type="text" id="order_table" name="order_table" value="{{$model->order_table}}"  autocomplete="off" class="layui-input">
					</div>
				</div>
...
<div class="my-btn-box">
	<span class="fl">
		...
		<a class="layui-btn btn-add btn-default layui-btn-sm" id="btn-add" href="{{ URL('admin/orderAndStatusType/create') }}">添加</a>
...
,{field: "order_table", title: "{{ $model->labels["order_table"] }}",  sort: true}
,{field: "order_type", title: "{{ $model->labels["order_type"] }}",  sort: true}
,{field: "order_status_type_id", title: "{{ $model->labels["order_status_type_id"] }}",  sort: true}
,{field: "create_user", title: "{{ $model->labels["create_user"] }}",  sort: true}
	...
	table.on("tool(dataTable)", function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		...
		if(layEvent === "del"){
			layer.confirm("真的删除吗？", function(index){
				...
				$.ajax({ 
					type: "GET",
					url: "{{URL("admin/orderAndStatusType/ajaxDestroy")}}/"+data.id, 
					dataType: "json",
			...
			layer.msg("编辑操作");
			window.location.href="{{ url("admin/orderAndStatusType/edit/") }}/"+data.id;
		}
	});
	// 刷新
	...
	$("#btn-delete-all").on("click", function () {
		...
		$.ajax({ 
			type: "GET",
			url: "{{URL("admin/orderAndStatusType/ajaxDestroy")}}/"+ids, 
			dataType: "json",
			success: function(data){
				if(data.code=="000")
				{
