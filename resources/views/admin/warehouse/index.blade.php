...
<!-- 		<a class="layui-btn btn-add btn-default layui-btn-sm" id="btn-add" href="{{ URL('admin/warehouse/create') }}">添加</a> -->
...
,{field: "contacts", title: "{{ $model->labels["contacts"] }}",  sort: true}
,{field: "tel", title: "{{ $model->labels["tel"] }}",  sort: true}
,{field: "status_text", title: "{{ $model->labels["status"] }}",  sort: true}
,{field: "start_time", title: "{{ $model->labels["start_time"] }}",  sort: true}
,{field: "end_time", title: "{{ $model->labels["end_time"] }}",  sort: true}
	...
	table.on("tool(dataTable)", function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		...
		if(layEvent === "del"){
			layer.confirm("真的删除吗？", function(index){
				...
				$.ajax({ 
					type: "GET",
					url: "{{URL("admin/warehouse/ajaxDestroy")}}/"+data.id, 
					dataType: "json",
			...
			layer.msg("编辑操作");
			window.location.href="{{ url("admin/warehouse/edit/") }}/"+data.id;
		}
	});
	// 刷新
	...
	$("#btn-delete-all").on("click", function () {
		...
		$.ajax({ 
			type: "GET",
			url: "{{URL("admin/warehouse/ajaxDestroy")}}/"+ids, 
			dataType: "json",
			success: function(data){
				if(data.code=="000")
				{
