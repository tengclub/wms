...
<fieldset class="layui-elem-field">
	<legend>{{ $group->group_name }}组 - 用户</legend>
...
layui.use("table", function(){
	...
	var tableIns = table.render({ //其它参数在此省略
		...
		, cols: [[ 
			...
			,{field: "user", title: "{{ $model->labels["user"] }}", width: 200, sort: true}
// 			, {fixed: "right", title: "操作", width: 150, align: "center", toolbar: "#barOption"} //这里的toolbar值是模板元素的选择器
	...
	table.on("tool(dataTable)", function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		...
		if(layEvent === "del"){
			layer.confirm("真的删除吗？", function(index){
				...
				$.ajax({ 
					type: "GET",
					url: "{{URL("admin/sysGroupUser/ajaxDestroy")}}/"+data.id, 
					dataType: "json",
			...
			layer.msg("编辑操作");
			window.location.href="{{ url("admin/sysGroupUser/edit/") }}/"+data.id;
		}
	});
	// 刷新
	...
	$("#btn-add").on("click", function () {
		...
		layer.open({
		  ...
		  content: ['{{ url("admin/sysUser/group",['gid'=>$group->id]) }}', 'no']
		});
	});
	// 批量删除
	$("#btn-delete-all").on("click", function () {
		...
		$.ajax({ 
			type: "GET",
			url: "{{URL("admin/sysGroupUser/ajaxDestroy")}}/"+ids, 
			dataType: "json",
			success: function(data){
				if(data.code=="000")
				{
