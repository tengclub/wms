...
<form class="layui-form" action="{{ Request::getRequestUri() }}"  method="POST" enctype="multipart/form-data">
	...
	<div class="layui-form-item">
        ...
        <div class="layui-input-inline">
        	 {!! App\Lib\Ehtml::select(App\Models\WhShelf::getIdList(),$model->wh_shelf_id,['name'=>'page[wh_shelf_id]','id'=>'wh_shelf_id','lay-filter'=>'wh_shelf_id','class'=>'form-control']) !!}
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="text"  value="{{ $model->code }}" name="page[code]" id="code" readonly="readonly"  lay-verify="title" autocomplete="off" placeholder="{{ $model->labels['code'] }}" class="layui-input">
	...
	<div class="layui-form-item">
        <div class="layui-input-block">
            ...
            <a class="layui-btn  layui-btn-primary" id="btn-add" href="{{ URL('admin/whUnit/index') }}">返回</a>
        </div>
    </div>
</form>
@section('bodyEnd')
<script>
	layui.use(['form'], function(){
		...
		form.on('select(wh_area_id)', function(data){
			loadDataShelf();
		});
		...
		function loadDataShelf(){  
		 	$.ajax({ 
				type: 'GET',
				url: '{{URL('util/ajaxWhShelfSelect')}}/'+$("#wh_area_id").val(), 
				dataType: 'text',
				data:null,
				success: function(obj){
					$("#wh_shelf_id").html(obj);
					form.render('select');
		      	}
	      	});
			return false;
