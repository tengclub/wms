...
<div class="my-btn-box">
	<span class="fl">
		...
		<a class="layui-btn btn-add btn-default layui-btn-sm" id="btn-add" href="{{ URL('admin/whArea/create') }}">添加</a>
...
,{field: "status_text", title: "{{ $model->labels["status"] }}",  sort: true}
	...
	table.on("tool(dataTable)", function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		...
		if(layEvent === "del"){
			layer.confirm("真的删除吗？", function(index){
				...
				$.ajax({ 
					type: "GET",
					url: "{{URL("admin/whArea/ajaxDestroy")}}/"+data.id, 
					dataType: "json",
			...
			layer.msg("编辑操作");
			window.location.href="{{ url("admin/whArea/edit/") }}/"+data.id;
		}
	});
	// 刷新
	...
	$("#btn-delete-all").on("click", function () {
		...
		$.ajax({ 
			type: "GET",
			url: "{{URL("admin/whArea/ajaxDestroy")}}/"+ids, 
			dataType: "json",
			success: function(data){
				if(data.code=="000")
				{
