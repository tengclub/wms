...
use App\Lib\Wechat\Pay\Lib\WxPayData;
...
class WxPayApi
{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public function mchPay($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsPartner_trade_noSet()){
			...
			throw new WxPayException("企业付款接口中，缺少必填参数spbill_create_ip！");
		}else if(!$inputObj->IsPartner_trade_noSet()){
			throw new WxPayException("企业付款接口中，缺少必填参数partner_trade_no！");
		}
		
		$inputObj->SetMch_appid($this->wxpaycfg->appid);//公众账号ID
		$inputObj->SetMchid($this->wxpaycfg->mchid);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		...
		$startTimeStamp = self::getMillisecond();//请求开始时间
		$response = self::postXmlCurl($xml, $url, true, $timeOut);
		$result = $this->wxpayresults->Init($response,false);
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
		return $result;
	}
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public function mchPayQuery($inputObj, $timeOut = 6)
	{
		$url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/gettransferinfo";
		//检测必填参数
		if(!$inputObj->IsPartner_trade_noSet()) 
			{
				throw new WxPayException("企业付款接口中，partner_trade_no参数必填！");
			}
			$inputObj->SetAppid($this->wxpaycfg->appid);//公众账号ID
			$inputObj->SetMch_id($this->wxpaycfg->mchid);//商户号
			$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
