...
class ErrorCode
{
    ...
    public static function getErrorText($err) {
        if (isset(self::$errorCode[$err])) {
            return self::$errorCode[$err];
        }else {
            return false;
        };
    }
    public static $payErrorCode=array(
    		'ORDERNOTEXIST '=>'查询系统中不存在此交易订单号',
    		'ORDERNOTEXIST '=>'查询系统中不存在此交易订单号',
    );
    
    public static function getPayErrorText($err) {
    	if (isset(self::$payErrorCode[$err])) {
    		return self::$payErrorCode[$err];
    	}else {
    		return false;
    	};
    }
