...
class HomeController
{
    ...
    public function index()
    {
    	...
    	{
    		$menu = WapMenu::orderBy('order_id', 'asc')->get();
    	}else {
    		$prefix = config('database')['connections']['mysql']['prefix'];
    		$where = ['pid'=>'0'];
    		$condition = 'id in(select menu_id from '.$prefix.'sys_group_wap_menu where group_id in(select group_id from '.$prefix.'sys_group_user where user="'.$user->user.'"))';
    		$menu = WapMenu::whereRaw($condition)->groupBy('id')->orderBy('order_id', 'asc')->get();
    	}

        return view('wap.home.index',['menu'=>$menu]);
    }
