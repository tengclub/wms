...
class OrderStatusLogController 
{
	...
	public function index(Request $request)
	{
		...
		{
			$_data = null;
    		$data = OrderStatusLog::whereRaw($condtion)->orderBy($request->get('field','id'),$request->get('order','desc'))->paginate($request->get('limit'));
    		foreach ($data as $boj) {
		...
		return view('admin.orderStatusLog.index', ['model'=>$model]);
	}
	/**
	...
	public function create()
	{
		...
		if(isset($_POST['page']))
		{
			...
			if ($validator->passes())
			{
				...
				if ($model->save()) {
					return view('admin.public.msgOk', ['msg'=>'保存成功','url'=>url('admin/orderStatusLog/index')]);
				} else {
		...
		return view('admin.orderStatusLog.create', ['model'=>$model]);
	}
	/**
	 * Show the form for creating a new resource.
	 * @param  int  $id
	...
	{
		$model = OrderStatusLog::findOrNew($id);
		if(isset($_POST['page']))
		{
			$page = $_POST['page'];
			$validator = Validator::make($page,
	...
	public function destroy($id)
	{
		...
		return redirect('admin/orderStatusLog/index');
	}
	/**
	 * Remove the specified resource from storage.
	 *
