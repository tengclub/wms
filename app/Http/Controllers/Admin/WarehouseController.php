...
class WarehouseController 
{
	...
	public function index(Request $request)
	{
		...
		{
			$_data = null;
    		$data = Warehouse::whereRaw($condtion)->orderBy($request->get('field','id'),$request->get('order','desc'))->paginate($request->get('limit'));
    		foreach ($data as $boj) {
		...
		return view('admin.warehouse.index', ['model'=>$model]);
	}
	/**
	...
	public function create()
	{
		...
		if(isset($_POST['page']))
		{
			...
			if ($validator->passes())
			{
				...
				if ($model->save()) {
					return view('admin.public.msgOk', ['msg'=>'保存成功','url'=>url('admin/warehouse/index')]);
				} else {
		...
		return view('admin.warehouse.create', ['model'=>$model]);
	}
	/**
	 * Show the form for creating a new resource.
	 * @param  int  $id
	...
	public function destroy($id)
	{
		...
		return redirect('admin/warehouse/index');
	}
	/**
	 * Remove the specified resource from storage.
	 *
