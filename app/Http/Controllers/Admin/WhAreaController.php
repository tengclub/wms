...
class WhAreaController 
{
	...
	public function index(Request $request)
	{
		...
		{
			$_data = null;
    		$data = WhArea::whereRaw($condtion)->orderBy($request->get('field','id'),$request->get('order','desc'))->paginate($request->get('limit'));
    		foreach ($data as $boj) {
		...
		return view('admin.whArea.index', ['model'=>$model]);
	}
	/**
	...
	public function create()
	{
		...
		if(isset($_POST['page']))
		{
			...
			$whArea = WhArea::firstOrNew(['warehouse_id'=>$model->warehouse_id,'code'=>$model->code]);
			if($whArea->id)
			{
				...
				if ($validator->passes())
				{
					...
					if ($model->save()) {
						return view('admin.public.msgOk', ['msg'=>'保存成功','url'=>url('admin/whArea/index')]);
					} else {
		...
		return view('admin.whArea.create', ['model'=>$model]);
	}
	/**
	 * Show the form for creating a new resource.
	 * @param  int  $id
	...
	public function destroy($id)
	{
		...
		return redirect('admin/whArea/index');
	}
	/**
	 * Remove the specified resource from storage.
	 *
